...
bool InstrumentStorage::baselineCaptured(StoredLoadSide side)
{
    if (!ready_) {
        return false;
    }

    return preferences_.getBool(
        key(side, "lb_ok", "rb_ok"),
        false
    );
}

bool InstrumentStorage::setBaselineCaptured(
    StoredLoadSide side,
    bool captured
)
{
    ...
    return preferences_.putBool(
        key(side, "lb_ok", "rb_ok"),
        captured
    ) > 0;
}

void InstrumentStorage::resetChannel(StoredLoadSide side)
{
    if (!ready_) {
        return;
    }
